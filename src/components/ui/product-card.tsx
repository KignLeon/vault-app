...
export function ProductCard({
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      <p className="font-mono text-[17px] md:text-[20px] tracking-[0.05em] font-bold leading-tight mt-2 px-2" style={{ color: fg }} onClick={() => onClick?.(product)}>
        {product.name}
      </p>

      {/* Price + Bulk starting */}
      <div className="flex items-baseline gap-2.5 mt-2 pb-3 px-2">
        <span className="font-mono text-[19px] md:text-[22px] font-bold" style={{ color: fg }}>${product.price}</span>
        {hasBulk && product.bulk && (
          <span className="font-mono text-[13px] tracking-wider" style={{ color: muted }}>
